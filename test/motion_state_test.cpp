...
TEST(MotionState, extrapolate_with_acceleration){
    //start at time t=2, with position 12, velocity 10, and acceleration 5
...
TEST(MotionState, invert){
   MotionState* a = new MotionState(10.0, 12.0, 13.0, 1.0); 
   ASSERT_EQ(10.0, a->get_pos());
   ASSERT_EQ(12.0, a->get_vel());
   ASSERT_EQ(13.0, a->get_accel());
   ASSERT_EQ(1.0, a->get_time());
   a->invert();
   ASSERT_EQ(-10.0, a->get_pos());
   ASSERT_EQ(-12.0, a->get_vel());
   ASSERT_EQ(-13.0, a->get_accel());
